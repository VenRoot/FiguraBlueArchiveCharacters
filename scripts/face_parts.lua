...
FaceParts = {
	...
	setEmotion = function (rightEye, leftEye, mouth, duration, force)
		...
		if FaceParts.EmotionCount == 0 or force then
			--右目
			if FaceParts.EyeUV[rightEye] ~= nil then
				rightEyePart:setUVPixels(FaceParts.EyeUV[rightEye][1], FaceParts.EyeUV[rightEye][2])
			end
			FaceParts.RightEyeStatus = rightEye
			--左目
			if FaceParts.EyeUV[leftEye] ~= nil then
				leftEyePart:setUVPixels(FaceParts.EyeUV[leftEye][1], FaceParts.EyeUV[leftEye][2])
			end
			--口
			if FaceParts.MouthUV[mouth] ~= nil then
				models.models.main.Avatar.Head.FaceParts.Mouth:setUVPixels(FaceParts.MouthUV[mouth][1], FaceParts.MouthUV[mouth][2])
			end
